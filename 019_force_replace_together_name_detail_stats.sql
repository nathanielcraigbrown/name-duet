-- Force-replaces the Together-mode stats RPC.
-- This removes the stale function body that referenced p.participant_name.
-- No names, votes, rankings, rooms, or participants are modified.

begin;

drop function if exists public.get_together_name_detail_stats(uuid, text);

create function public.get_together_name_detail_stats(
  p_access_token uuid,
  p_display_name text
)
returns jsonb
...
begin
  ...
  with room_participants as (
    ...
    select
      ...
      case when coalesce(r.comparisons, 0) > 0
