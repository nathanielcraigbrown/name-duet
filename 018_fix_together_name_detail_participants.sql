-- Fixes Together-mode stats on schemas where participants does not have
-- a participant_name column. Uses JSON field lookup so this remains compatible
-- with name/display_name/participant_name variants.
  ...
  with room_participants as (
    select
      ...
      coalesce(
        ...
        case when p.id = requester.id then 'You' else 'Partner' end
    ...
    select jsonb_build_object(
      ...
      'participant_name', ps.participant_label,
